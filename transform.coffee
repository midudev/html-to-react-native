...
Transform = ( fn ) ->
  ...
  return {
    ...
    text : ( el, style = {} ) ->
      text = decodeHTMLEntities( _s.strip( el.raw ) )
      "<Text style={#{style}}>#{text}</Text>"

    image : ( el ) ->
      if el.src? then "<Image source={{uri: '#{el.src}'}} />"
      else ""

    bold : ( el ) ->
      fn( el.children, '', { fontWeight: 'bold' } )

    paragraph : ( el ) ->
      style = { fontWeight: '200', color: '#111' }
      fn( el.children, '', style ) + '\n\n'

    heading : ( el ) ->
      style = { fontSize: 20, fontWeight: '200', color: '#111' }
      fn( el.children, style )
